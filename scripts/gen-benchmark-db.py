...
def GenerateFiles (fileCount = 100000, featureCount=64, packageCount=4096):
    ...
    for featureIndex in range (featureCount):
        ...
        for fileIndex in range (fileCount // featureCount):
            ...
            # one content object is split between 1..4 packages
            for i in range (random.randrange (1, 4)):
                c.execute ('''INSERT INTO storage_mapping VALUES (?, ?)''',
                (contentObjectId, random.randrange (packageCount)))
    conn.commit ()
    conn.close ()
